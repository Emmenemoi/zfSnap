...
# List local snapshots of {1: dataset}
ListSnapshots() {
    local zfs_snapshots
...
# get snapshot stay time
getStayTime() {
    local stay_time=$(TTL2Seconds `echo "$1" | $ESED -e "s/^.*@//" | $ESED -e "s/^(${prefixes})?${date_pattern}--//"`)
    echo "$stay_time"
}

# get snapshot creation timestamp
getCreationTime() {
    local create_time=$(Date2Timestamp `echo "$1" | $ESED -e "s/^.*@//" | $ESED -e "s/--${htime_pattern}$//; s/^(${prefixes})?//"`)
    echo "$create_time"
}
